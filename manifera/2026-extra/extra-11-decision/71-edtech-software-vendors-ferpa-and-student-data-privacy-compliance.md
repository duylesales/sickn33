...
## By The Numbers: What a Real FERPA Due-Diligence File Contains

A compliance file that would actually hold up under a district audit or a parent complaint investigation needs more than a signed DPA. Concretely, it should contain:

- **The signed data processing agreement itself**, with the school-official-exception language (direct control, purpose limitation, redisclosure prohibition) highlighted and cross-referenced to 34 CFR §99.31(a)(1) — not a generic vendor terms-of-service page.
- **A dated subprocessor list**, refreshed at least annually or on material change, naming every entity — including AI model providers — with access to student data, not a category description like "cloud infrastructure providers."
- **State-law applicability mapping**, listing every state where enrolled students reside and the specific statute obligations that apply there (NY Ed Law §2-d, IL SOPPA, CA SOPIPA, and others), since a single national FERPA checklist misses roughly a dozen state-specific requirements most districts operate under today.
- **A written deletion certification template**, with the contractual deletion window (commonly 30-90 days) and explicit confirmation it covers backup/DR copies, not just primary storage.
- **AI feature disclosure**, if applicable: retention window, training-use confirmation or denial, and the underlying LLM provider's data handling terms, since several state frameworks now require this explicitly.

A vendor that can't populate all five sections on request within a normal sales cycle is not audit-ready, regardless of what their marketing claims.
...
### (Scenario: A compliance officer is reviewing a vendor whose product added an AI writing assistant mid-contract) What should we do if a vendor adds a generative AI feature to a product we're already under contract for?
Treat it as a material change requiring a new privacy review, not an automatic extension of the existing DPA — ask specifically whether student inputs to the new AI feature are sent to a third-party LLM provider, whether that provider retains or trains on the data, and whether your existing subprocessor list and data-element disclosure need amending before the feature is enabled for your students.

### (Scenario: A district's compliance officer is deciding between a vendor's own NDPA-based DPA and the district's standard template) Should we accept a vendor's own NDPA exhibits or insist on our district's standard data privacy addendum?
A vendor's registered NDPA is acceptable groundwork, but you should still overlay your district's or state's specific addendum requirements where they exceed the NDPA baseline — the NDPA is a floor negotiated across many agencies, and some states or districts have negotiated stricter breach-notification timelines or audit rights that a generic NDPA exhibit won't include.

### (Scenario: A compliance officer discovers a vendor's subprocessor list hasn't been updated in over a year) How often should a vendor's subprocessor list actually be refreshed, and what should trigger an off-cycle update?
At minimum annually, but any material change — adding a new AI model provider, switching cloud infrastructure regions, or onboarding a new analytics subprocessor — should trigger an off-cycle notification under the DPA, not wait for the annual cycle. A vendor unable to describe this update trigger contractually likely isn't tracking subprocessor changes rigorously internally either.

### (Scenario: A parent files a complaint alleging a vendor shared their child's data with an advertiser) What documentation should a compliance officer have ready to respond to a parent complaint about vendor data sharing?
The signed DPA showing the school-official-exception language and redisclosure prohibition, the vendor's subprocessor list at the time of the alleged incident, and any state-law-specific disclosure (like SOPPA's required list of collected data elements) that confirms what the vendor was contractually permitted to do — without these on file in advance, responding to a parent complaint becomes a reactive vendor interrogation instead of a quick documentation check.
...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    ...
    {"@type": "Question", "name": "(Scenario: A compliance officer is reviewing a vendor whose product added an AI writing assistant mid-contract) What should we do if a vendor adds a generative AI feature to a product we're already under contract for?", "acceptedAnswer": {"@type": "Answer", "text": "Treat it as a material change requiring a new privacy review, not an automatic extension of the existing DPA — ask specifically whether student inputs to the new AI feature are sent to a third-party LLM provider, whether that provider retains or trains on the data, and whether your existing subprocessor list and data-element disclosure need amending before the feature is enabled for your students."}},
    {"@type": "Question", "name": "(Scenario: A district's compliance officer is deciding between a vendor's own NDPA-based DPA and the district's standard template) Should we accept a vendor's own NDPA exhibits or insist on our district's standard data privacy addendum?", "acceptedAnswer": {"@type": "Answer", "text": "A vendor's registered NDPA is acceptable groundwork, but you should still overlay your district's or state's specific addendum requirements where they exceed the NDPA baseline — the NDPA is a floor negotiated across many agencies, and some states or districts have negotiated stricter breach-notification timelines or audit rights that a generic NDPA exhibit won't include."}},
    {"@type": "Question", "name": "(Scenario: A compliance officer discovers a vendor's subprocessor list hasn't been updated in over a year) How often should a vendor's subprocessor list actually be refreshed, and what should trigger an off-cycle update?", "acceptedAnswer": {"@type": "Answer", "text": "At minimum annually, but any material change — adding a new AI model provider, switching cloud infrastructure regions, or onboarding a new analytics subprocessor — should trigger an off-cycle notification under the DPA, not wait for the annual cycle. A vendor unable to describe this update trigger contractually likely isn't tracking subprocessor changes rigorously internally either."}},
    {"@type": "Question", "name": "(Scenario: A parent files a complaint alleging a vendor shared their child's data with an advertiser) What documentation should a compliance officer have ready to respond to a parent complaint about vendor data sharing?", "acceptedAnswer": {"@type": "Answer", "text": "The signed DPA showing the school-official-exception language and redisclosure prohibition, the vendor's subprocessor list at the time of the alleged incident, and any state-law-specific disclosure (like SOPPA's required list of collected data elements) that confirms what the vendor was contractually permitted to do — without these on file in advance, responding to a parent complaint becomes a reactive vendor interrogation instead of a quick documentation check."}}
  ]
}
</script>
