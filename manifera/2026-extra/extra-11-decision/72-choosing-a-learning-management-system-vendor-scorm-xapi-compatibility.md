...
## Implementation Checklist: The Migration Trial Nobody Runs Until It's Too Late

Before signing, run a scoped proof-of-concept migration — not a feature checklist review — with these specific checks:

- **Package fidelity:** upload 3-5 representative SCORM packages, including at least one SCORM 2004 package with real branching logic, and confirm completion status, score, and time-tracked-in-content report identically to your current LMS.
- **xAPI export format:** request a full statement export in raw xAPI JSON, not a CSV summary — verify the actor/verb/object structure is preserved and importable into a third-party LRS, since a "yes we support export" answer that turns out to be a proprietary CSV dump defeats the portability purpose entirely.
- **LTI 1.3 live test:** launch at least one existing third-party tool (a publisher's content, a proctoring tool) through the candidate LMS's LTI 1.3 Advantage connection and confirm grade passback lands correctly in the gradebook, not just that SSO succeeds.
- **Roster sync dry run:** if OneRoster applies, run one full term-start sync cycle in the sandbox and manually verify section, teacher, and student mappings against your SIS export — this is where silent mapping errors surface.
- **Exit clause:** confirm contractually that you can re-export all content and tracking data, in the same formats you imported, at contract termination without a professional services fee.

Budget 2-3 weeks for this trial; vendors that can't support a sandbox this thorough are signaling the same friction at your actual migration.
...
### (Scenario: An IT manager finds a vendor's xAPI export only produces a summary CSV, not raw statements) Is a CSV export of xAPI activity data an acceptable substitute for a real xAPI statement export?
No — a CSV summary discards the actor-verb-object statement structure that makes xAPI portable to another LRS or reporting tool in the first place. Insist on export in native xAPI JSON statement format; a vendor offering only CSV has effectively built a proprietary reporting layer with an xAPI label on it.

### (Scenario: A corporate L&D buyer is comparing an LMS against a standalone LRS product) Do we need a dedicated LRS product, or can the LMS's built-in xAPI store handle informal and on-the-job learning tracking too?
It depends on whether you need statements from sources the LMS itself never launches — a simulation, a field-service checklist app, a mobile microlearning tool. If those exist or are planned, a vendor-independent LRS that any conformant system can send statements to avoids re-litigating this integration with every new content source.

### (Scenario: A vendor claims LTI 1.3 Advantage support but the sales team can't answer questions about Assignment and Grade Services) How do we verify LTI 1.3 Advantage support isn't just LTI 1.3 authentication without the Advantage extensions?
Ask for a live launch of an existing third-party tool through the sandbox and confirm gradebook passback happens automatically via Assignment and Grade Services — not a manual grade entry workaround. A vendor that supports only the base LTI 1.3 security layer without Advantage's deep linking and grade services will require manual gradebook work despite technically being "LTI 1.3 compliant."

### (Scenario: A district is mid-negotiation and the LMS vendor wants to charge extra for a full content export at contract end) Should a full data and content export at contract termination be a paid professional services engagement?
No — a vendor confident in genuine interoperability builds standard-format export into the base platform and contract terms, since the whole point of SCORM/xAPI/LTI compliance is portability. Treat a per-incident export fee at termination as a red flag and negotiate it out of the contract before signing, not after you need to leave.
...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    ...
    {"@type": "Question", "name": "(Scenario: An IT manager finds a vendor's xAPI export only produces a summary CSV, not raw statements) Is a CSV export of xAPI activity data an acceptable substitute for a real xAPI statement export?", "acceptedAnswer": {"@type": "Answer", "text": "No — a CSV summary discards the actor-verb-object statement structure that makes xAPI portable to another LRS or reporting tool in the first place. Insist on export in native xAPI JSON statement format; a vendor offering only CSV has effectively built a proprietary reporting layer with an xAPI label on it."}},
    {"@type": "Question", "name": "(Scenario: A corporate L&D buyer is comparing an LMS against a standalone LRS product) Do we need a dedicated LRS product, or can the LMS's built-in xAPI store handle informal and on-the-job learning tracking too?", "acceptedAnswer": {"@type": "Answer", "text": "It depends on whether you need statements from sources the LMS itself never launches — a simulation, a field-service checklist app, a mobile microlearning tool. If those exist or are planned, a vendor-independent LRS that any conformant system can send statements to avoids re-litigating this integration with every new content source."}},
    {"@type": "Question", "name": "(Scenario: A vendor claims LTI 1.3 Advantage support but the sales team can't answer questions about Assignment and Grade Services) How do we verify LTI 1.3 Advantage support isn't just LTI 1.3 authentication without the Advantage extensions?", "acceptedAnswer": {"@type": "Answer", "text": "Ask for a live launch of an existing third-party tool through the sandbox and confirm gradebook passback happens automatically via Assignment and Grade Services — not a manual grade entry workaround. A vendor that supports only the base LTI 1.3 security layer without Advantage's deep linking and grade services will require manual gradebook work despite technically being \"LTI 1.3 compliant.\""}},
    {"@type": "Question", "name": "(Scenario: A district is mid-negotiation and the LMS vendor wants to charge extra for a full content export at contract end) Should a full data and content export at contract termination be a paid professional services engagement?", "acceptedAnswer": {"@type": "Answer", "text": "No — a vendor confident in genuine interoperability builds standard-format export into the base platform and contract terms, since the whole point of SCORM/xAPI/LTI compliance is portability. Treat a per-incident export fee at termination as a red flag and negotiate it out of the contract before signing, not after you need to leave."}}
  ]
}
</script>
