...
## By the Numbers: Setting a Real SLA for RPM Vulnerability Response

A defined patch SLA is the difference between an operational security program and a policy document: reasonable industry benchmarks are critical vulnerabilities mitigated or patched within 15-30 days of disclosure, high-severity within 30-60 days, and lower-severity issues addressed within the next scheduled release cycle. Ask any RPM vendor for their actual SLA numbers and their track record against them over the past 12 months, not just the target — a vendor who can produce a specific "we patched X of our last Y critical CVEs within our stated window" figure has a real program; one who can only describe the policy has not yet had to test it under pressure.

Device-side patching is the harder number to get honest data on: firmware updates for fielded RPM devices in patient homes typically take materially longer to roll out than cloud-side patches, often weeks to months depending on connectivity requirements and whether the update needs a caregiver or clinical staff visit to complete. For a program with a few thousand devices in the field, ask what percentage of the fleet the vendor can realistically reach with a critical firmware patch within 30 days — a vendor without device-level remote update capability may be looking at a multi-month exposure window for any firmware-level vulnerability, which is a materially different risk profile than a cloud-only patch cycle suggests.
