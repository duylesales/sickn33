...
## Technical Deep-Dive: How CLDR Plural Rules Actually Break Naive Code

Take the sentence "You have 3 items in your cart." In English, this needs exactly two branches: one item, or other. A vendor writing `count === 1 ? "item" : "items"` has built code that cannot localize into Arabic, which requires six CLDR plural categories (zero, one, two, few, many, other) each mapped to different numeric ranges — "few" applies to numbers ending in 3-10 when not preceded by 11-99, a rule no English-first engineer would guess. Russian needs three categories with its own boundary logic (one, few, many), and Polish needs four. ICU MessageFormat solves this by letting translators supply a full plural rule set per locale — `{count, plural, one {# item} few {# items} many {# items} other {# items}}` — evaluated at runtime against the correct CLDR category for that locale, not hardcoded English logic. The practical test for a vendor: ask them to localize a simple counter string into Arabic during the discovery call. A vendor fluent in this will immediately ask which CLDR categories your content requires; a translation-first vendor will ask which words to use for "item" and "items" and stop there, missing four of the six branches Arabic actually needs.
...
### (Scenario: our product already shipped in English-only and now needs a retrofit for German and Arabic) Is it cheaper to retrofit i18n into an existing codebase or would a rewrite be more cost-effective?
Retrofitting is almost always cheaper than a rewrite, but only if scoped honestly upfront — ask the vendor to audit the codebase first and quantify how much text is hardcoded inline versus already externalized, since that ratio drives the estimate more than raw feature count. A vendor who quotes a fixed price without this audit is guessing, and retrofits with unaudited scope are the most common source of localization budget overruns.

### (Scenario: our design system was built entirely for English and never anticipated RTL) What's the actual engineering cost of adding RTL support after the fact versus designing for it from the start?
Adding RTL after a design system is fully built typically means auditing every component for hardcoded left/right values and directional icon assumptions, then rebuilding the ones that fail — a cost that's substantially lower if logical CSS properties were used from the start, since the swap becomes largely automatic rather than component-by-component. Ask the vendor to name specifically which components (icons, progress indicators, form layouts) will need manual RTL variants versus which will flip automatically.

### (Scenario: CMO is comparing a vendor's per-word translation rate against a competitor's flat project fee) How do we compare pricing between a translation agency and a localization engineering vendor fairly?
Per-word rates only price the linguistic layer, so a low per-word quote from a translation-only vendor is not comparable to a flat project fee that includes i18n engineering, RTL audit, and locale QA — ask both vendors to itemize engineering hours separately from translation volume so the comparison is apples to apples. The engineering layer, not the per-word rate, is usually where the real cost difference between vendors lives.

### (Scenario: we're launching in the Netherlands, Germany, and France simultaneously and need to prioritize QA effort under time pressure) Which locale QA checks matter most when we can't do a full review cycle on every market before launch?
Prioritize pseudo-localization layout checks first, since text expansion and container overflow are the most visible failure and the cheapest to catch early, then run in-context linguistic review only on the flagship market if time is genuinely constrained. Never skip number and date formatting QA — Germany, France, and the Netherlands all use different decimal separators and date orders despite being geographically close, and that specific error erodes trust immediately at checkout or in any form.
...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    ...
    {"@type": "Question", "name": "(Scenario: our product already shipped in English-only and now needs a retrofit for German and Arabic) Is it cheaper to retrofit i18n into an existing codebase or would a rewrite be more cost-effective?", "acceptedAnswer": {"@type": "Answer", "text": "Retrofitting is almost always cheaper than a rewrite, but only if scoped honestly upfront — ask the vendor to audit the codebase first and quantify how much text is hardcoded inline versus already externalized, since that ratio drives the estimate more than raw feature count."}},
    {"@type": "Question", "name": "(Scenario: our design system was built entirely for English and never anticipated RTL) What's the actual engineering cost of adding RTL support after the fact versus designing for it from the start?", "acceptedAnswer": {"@type": "Answer", "text": "Adding RTL after a design system is fully built typically means auditing every component for hardcoded left/right values and directional icon assumptions, then rebuilding the ones that fail — substantially cheaper if logical CSS properties were used from the start. Ask the vendor to name which components need manual RTL variants versus which flip automatically."}},
    {"@type": "Question", "name": "(Scenario: CMO is comparing a vendor's per-word translation rate against a competitor's flat project fee) How do we compare pricing between a translation agency and a localization engineering vendor fairly?", "acceptedAnswer": {"@type": "Answer", "text": "Per-word rates only price the linguistic layer, so a low per-word quote from a translation-only vendor isn't comparable to a flat project fee that includes i18n engineering, RTL audit, and locale QA — ask both vendors to itemize engineering hours separately from translation volume."}},
    {"@type": "Question", "name": "(Scenario: we're launching in the Netherlands, Germany, and France simultaneously and need to prioritize QA effort under time pressure) Which locale QA checks matter most when we can't do a full review cycle on every market before launch?", "acceptedAnswer": {"@type": "Answer", "text": "Prioritize pseudo-localization layout checks first since text expansion is the most visible failure and cheapest to catch, then run in-context linguistic review on the flagship market if time is constrained. Never skip number and date formatting QA — these three markets each use different decimal separators and date orders despite being geographically close."}}
  ]
}
</script>
