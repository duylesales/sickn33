...
## Implementation Checklist: Vetting a Design System Proposal Line by Line

Before signing, walk the vendor's proposal against this checklist rather than judging it on the strength of the component showcase alone. Token layer: does the proposal name primitive-to-semantic separation explicitly, and is Figma variables or Tokens Studio specified as the delivery format (a PDF style guide fails this immediately)? Governance: does the SOW name a specific cadence (biweekly is standard) for a design-engineering review group, and does it define semver rules for the component package? Cross-platform: if you ship iOS or Android, does the proposal name Style Dictionary or an equivalent token pipeline, and does it separately scope per-platform component implementation rather than assuming one codebase covers three platforms? Documentation: is Storybook (or a named equivalent) a line item with its own review cycle, not a byproduct of the build phase? Adoption: does the SOW include a migration guide, a defined support window post-handoff, and a tracked adoption metric (percentage of screens on system components) rather than treating delivery as the finish line? A proposal missing three or more of these five items is scoped as a component showcase, not a design system — expect the 80% that determines long-term adoption to land back on your internal team's plate within two quarters.
...
### (Scenario: our engineering team wants to publish the component library as an internal npm package) What should we require from the vendor around package versioning before they hand off?
Require the vendor to establish semantic versioning from the first release — patch for fixes, minor for backward-compatible additions, major for breaking changes — and to document a deprecation policy for old component variants before they're removed. Without this in place at handoff, your internal team inherits an unversioned codebase where every update requires manually diffing changes across every consuming team.

### (Scenario: three product teams have already forked their own versions of the same dropdown component) Can a design system vendor help us consolidate existing fragmentation, or only build new systems from scratch?
A credible vendor will start a design system engagement with an audit phase that inventories existing component variants across teams before proposing a consolidated set, rather than ignoring the fragmentation and building a fourth version. Ask specifically whether audit and consolidation work is scoped into the engagement or treated as a separate, unbudgeted phase.

### (Scenario: our product needs both a light theme and a white-labeled variant for an enterprise client) Does semantic token architecture actually support white-labeling, or is that a separate build?
A properly separated primitive-to-semantic token architecture supports white-labeling by swapping the primitive values a client's semantic tokens reference, without touching component code — this is the exact reason the separation matters beyond dark mode. Ask the vendor to demo a second theme applied to an existing component during evaluation, not just describe the architecture conceptually.

### (Scenario: Head of Product needs to justify design system spend to finance) What adoption metric should we report back to leadership to prove the investment paid off?
Track the percentage of shipped screens using system components versus legacy one-offs, measured quarterly against the 2-4 quarter adoption timeline, alongside a rough estimate of engineering hours saved per new feature once teams stop building one-off components. A vendor who scoped adoption tracking into the engagement should hand you a baseline measurement at launch to compare against.
...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    ...
    {"@type": "Question", "name": "(Scenario: our engineering team wants to publish the component library as an internal npm package) What should we require from the vendor around package versioning before they hand off?", "acceptedAnswer": {"@type": "Answer", "text": "Require the vendor to establish semantic versioning from the first release — patch for fixes, minor for additions, major for breaking changes — and to document a deprecation policy for old component variants before they're removed. Without this, your internal team inherits an unversioned codebase where every update requires manually diffing changes."}},
    {"@type": "Question", "name": "(Scenario: three product teams have already forked their own versions of the same dropdown component) Can a design system vendor help us consolidate existing fragmentation, or only build new systems from scratch?", "acceptedAnswer": {"@type": "Answer", "text": "A credible vendor will start with an audit phase that inventories existing component variants across teams before proposing a consolidated set, rather than ignoring the fragmentation and building a fourth version. Ask whether audit and consolidation work is scoped into the engagement or treated as a separate, unbudgeted phase."}},
    {"@type": "Question", "name": "(Scenario: our product needs both a light theme and a white-labeled variant for an enterprise client) Does semantic token architecture actually support white-labeling, or is that a separate build?", "acceptedAnswer": {"@type": "Answer", "text": "A properly separated primitive-to-semantic token architecture supports white-labeling by swapping the primitive values a client's semantic tokens reference, without touching component code. Ask the vendor to demo a second theme applied to an existing component during evaluation, not just describe the architecture conceptually."}},
    {"@type": "Question", "name": "(Scenario: Head of Product needs to justify design system spend to finance) What adoption metric should we report back to leadership to prove the investment paid off?", "acceptedAnswer": {"@type": "Answer", "text": "Track the percentage of shipped screens using system components versus legacy one-offs, measured quarterly against the 2-4 quarter adoption timeline, alongside a rough estimate of engineering hours saved per new feature once teams stop building one-off components."}}
  ]
}
</script>
