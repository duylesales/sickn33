...
## The Go/No-Go Checklist: Objective Cutover Criteria

A migration should never proceed to DNS cutover on a subjective "looks fine." Require the vendor to sign off against objective, pre-agreed criteria like these before go-live:

1. **Redirect map coverage.** 100% of URLs from the pre-migration Screaming Frog crawl mapped to a specific new-platform destination — zero blanket homepage redirects, zero unmapped URLs left as 404s.
2. **Checkout pass rate.** Every payment method actually used by your customer base (iDEAL, credit card, and any B2B invoice flow) completing a full test transaction on the new platform, including a webhook-confirmed order record.
3. **Load test threshold.** New platform holding response times under 2-3 seconds at a minimum of 3x average daily peak traffic, tested against a realistic checkout-heavy load pattern, not just homepage requests.
4. **DNS TTL lowered.** Confirmed at 300 seconds or less, at least 24-48 hours before cutover, verified independently rather than taken on the vendor's word.
5. **Rollback rehearsal completed.** A documented dry run of reverting DNS back to the old platform, with the actual time-to-restore measured, not estimated.

Any one of these unmet is a legitimate reason to delay cutover — a vendor who pushes back on delaying for an unmet criterion is prioritizing their delivery date over your revenue.
...
### (Scenario: migration scheduled right before a major promotional campaign) Marketing wants the new platform live before a major seasonal campaign launches in three weeks — is that enough time to migrate safely, or should we push the campaign?
Three weeks is workable only if redirect mapping, payment testing, and load testing are already substantially complete — cutover itself should never be scheduled inside or immediately before a known high-traffic window, since it concentrates rollback risk at the worst possible moment. If any of the go/no-go criteria (redirect coverage, checkout pass rate, load test results) are still open a week out, push the cutover date, not the testing.

### (Scenario: discovering iDEAL wasn't tested until after cutover) We migrated to a new platform over the weekend and Monday morning support tickets show iDEAL payments failing for Dutch customers — what went wrong and how do we prevent it next time?
This is a classic sign that payment method testing focused on credit cards and skipped explicit iDEAL validation, despite it being the dominant payment method in Dutch e-commerce — a gap a rigorous pre-cutover checklist would have caught. For the immediate fix, verify the new webhook endpoint and API credentials are correctly configured for the iDEAL integration specifically, not just the general payment gateway connection.

### (Scenario: organic traffic dropping sharply days after migration) Our organic traffic dropped 40% in the week after our platform migration — is this something we can still fix, or is the SEO damage permanent?
It's very often recoverable if caught quickly: check Google Search Console immediately for crawl errors and identify which URLs are 404ing or landing on a blanket homepage redirect instead of their specific new-platform equivalent, then correct the redirect map for those URLs. Rankings typically recover over several weeks once correct 301 redirects are in place, though the faster the gap is found and fixed, the less cumulative ranking damage accumulates.

### (Scenario: vendor wants to skip the rollback rehearsal to save time) Our migration vendor says a rollback rehearsal isn't necessary since the new platform has already been tested thoroughly — should we push back?
Push back — a tested platform and a rehearsed rollback answer different questions, since the rehearsal validates how quickly and cleanly you can revert if something unexpected surfaces specifically during the live cutover, not during controlled pre-launch testing. Skipping this step means the first time anyone finds out how long a real rollback takes is during an actual live incident, which is the worst possible moment to discover it takes longer than expected.
...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    ...
    {"@type": "Question", "name": "(Scenario: migration scheduled right before a major promotional campaign) Marketing wants the new platform live before a major seasonal campaign launches in three weeks — is that enough time to migrate safely, or should we push the campaign?", "acceptedAnswer": {"@type": "Answer", "text": "Three weeks is workable only if redirect mapping, payment testing, and load testing are already substantially complete — cutover itself should never be scheduled inside or immediately before a known high-traffic window. If any go/no-go criteria are still open a week out, push the cutover date, not the testing."}},
    {"@type": "Question", "name": "(Scenario: discovering iDEAL wasn't tested until after cutover) We migrated to a new platform over the weekend and Monday morning support tickets show iDEAL payments failing for Dutch customers — what went wrong and how do we prevent it next time?", "acceptedAnswer": {"@type": "Answer", "text": "This is a classic sign that payment testing focused on credit cards and skipped explicit iDEAL validation, despite it being the dominant payment method in Dutch e-commerce. For the immediate fix, verify the new webhook endpoint and API credentials are correctly configured for the iDEAL integration specifically, not just the general gateway connection."}},
    {"@type": "Question", "name": "(Scenario: organic traffic dropping sharply days after migration) Our organic traffic dropped 40% in the week after our platform migration — is this something we can still fix, or is the SEO damage permanent?", "acceptedAnswer": {"@type": "Answer", "text": "It's often recoverable if caught quickly: check Google Search Console for crawl errors, identify which URLs are 404ing or landing on a blanket homepage redirect instead of their specific equivalent, then correct the redirect map for those URLs. Rankings typically recover over several weeks once correct 301 redirects are in place."}},
    {"@type": "Question", "name": "(Scenario: vendor wants to skip the rollback rehearsal to save time) Our migration vendor says a rollback rehearsal isn't necessary since the new platform has already been tested thoroughly — should we push back?", "acceptedAnswer": {"@type": "Answer", "text": "Push back — a tested platform and a rehearsed rollback answer different questions, since the rehearsal validates how quickly you can revert if something unexpected surfaces during the live cutover specifically, not during controlled pre-launch testing. Skipping it means the first real test of rollback speed happens during an actual live incident."}}
  ]
}
</script>
