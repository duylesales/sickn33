...
## Benchmark Numbers to Hold a Vendor's Proposal Against

Use these reference figures to sanity-check a vendor's onboarding proposal — they should be able to state a target for each, not just describe the flow qualitatively:

- **Seller onboarding completion.** A well-designed flow with progressive profiling typically holds completion in the 55-70% range from start to first published listing; a flow with no progressive profiling commonly sees the 40% completion rate that makes the "60% never finish" statistic true.
- **Time-to-first-relevant-result for buyers.** Under 30 seconds from signup to a search returning genuinely relevant results is a reasonable target; anything requiring more than two search attempts before relevance is a liquidity-routing problem, not a search-tuning one.
- **Concierge-onboarded seller ratio at launch.** Marketplaces solving cold-start well often manually onboard the first 20-50 sellers in a launch category before opening self-serve signup — ask whether the vendor's plan accounts for this operational, non-engineering effort at all.
- **Payout setup drop-off.** Expect a measurable spike in abandonment at payout/KYC setup specifically — instrumentation should isolate this step from general listing-creation abandonment, since the causes and fixes differ.
- **Zero-result search rate.** Track this explicitly as a launch-readiness gate; a category shouldn't open to buyer signup while its zero-result rate exceeds roughly 15-20% of searches.
...
### (Scenario: launching a new category with zero existing sellers) We're about to open a brand-new product category with no existing sellers yet — should we let buyers discover it organically or hold off on promoting it?
Hold off on directing buyer traffic to a category until it clears a reasonable liquidity threshold, since opening an empty or thin category to buyers produces the hollow-search experience that drives buyer churn on the very first visit. Concierge-onboard an initial batch of sellers in that category first, and use the zero-result search rate as an explicit gate before promoting it broadly.

### (Scenario: seller drop-off concentrated at payout setup) Our funnel data shows sellers are completing listing creation but dropping off heavily at the payout account setup step — is this a UX problem or something else?
This is a common and specific friction point tied to the KYC/payout onboarding flow itself (often a Stripe Connect or equivalent embedded flow), not general form fatigue, so isolate it in your instrumentation and investigate separately from listing-creation drop-off. Consider deferring payout setup until after the seller's first listing gets real interest, using visible momentum to motivate completing the more sensitive verification step.

### (Scenario: vendor proposes building both signup flows in parallel with no sequencing plan) Our shortlisted vendor's proposal describes building "a seller signup flow" and "a buyer signup flow" as two parallel workstreams with no mention of which side launches first — is that a red flag?
Yes — this is the clearest sign a vendor is treating marketplace onboarding as two single-sided SaaS signups rather than solving the cold-start problem, and it typically produces a marketplace where both sides launch into emptiness. Ask them directly to propose a sequencing strategy (geographic, category-based, or concierge-seeded) before signing, and treat a vague answer as a scoping gap to resolve, not a detail to work out later.

### (Scenario: comparing funnel analytics platforms for a two-sided product) We're deciding between Amplitude, Mixpanel, and PostHog for our marketplace's funnel instrumentation — does the choice matter much for a two-sided product specifically?
The specific tool matters less than whether the implementation cleanly separates seller-funnel and buyer-funnel events from day one, since a shared, undifferentiated event stream makes it hard to diagnose which side is actually causing a given liquidity problem. Confirm with your vendor that their event schema distinguishes seller and buyer journeys explicitly, regardless of which platform you choose.
...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    ...
    {"@type": "Question", "name": "(Scenario: launching a new category with zero existing sellers) We're about to open a brand-new product category with no existing sellers yet — should we let buyers discover it organically or hold off on promoting it?", "acceptedAnswer": {"@type": "Answer", "text": "Hold off on directing buyer traffic until the category clears a reasonable liquidity threshold, since opening an empty or thin category to buyers produces the hollow-search experience that drives buyer churn on the first visit. Concierge-onboard an initial batch of sellers first, and use the zero-result search rate as an explicit gate before promoting it broadly."}},
    {"@type": "Question", "name": "(Scenario: seller drop-off concentrated at payout setup) Our funnel data shows sellers are completing listing creation but dropping off heavily at the payout account setup step — is this a UX problem or something else?", "acceptedAnswer": {"@type": "Answer", "text": "This is a common friction point tied to the KYC/payout onboarding flow itself, not general form fatigue, so isolate it in your instrumentation and investigate separately from listing-creation drop-off. Consider deferring payout setup until after the seller's first listing gets real interest, using visible momentum to motivate completing verification."}},
    {"@type": "Question", "name": "(Scenario: vendor proposes building both signup flows in parallel with no sequencing plan) Our shortlisted vendor's proposal describes building \"a seller signup flow\" and \"a buyer signup flow\" as two parallel workstreams with no mention of which side launches first — is that a red flag?", "acceptedAnswer": {"@type": "Answer", "text": "Yes — this is the clearest sign a vendor is treating marketplace onboarding as two single-sided SaaS signups rather than solving the cold-start problem. Ask them directly to propose a sequencing strategy before signing, and treat a vague answer as a scoping gap to resolve, not a detail to work out later."}},
    {"@type": "Question", "name": "(Scenario: comparing funnel analytics platforms for a two-sided product) We're deciding between Amplitude, Mixpanel, and PostHog for our marketplace's funnel instrumentation — does the choice matter much for a two-sided product specifically?", "acceptedAnswer": {"@type": "Answer", "text": "The specific tool matters less than whether the implementation cleanly separates seller-funnel and buyer-funnel events from day one, since a shared, undifferentiated event stream makes it hard to diagnose which side is causing a given liquidity problem. Confirm your vendor's event schema distinguishes seller and buyer journeys explicitly, regardless of platform."}}
  ]
}
</script>
