...
## The Technical Due Diligence Scorecard

Score a shortlisted data engineering vendor against these five specifics during the technical interview — vague answers on more than one should reset your confidence in the overall pitch:

1. **CI test count, not test existence.** Ask "how many dbt tests run in your CI pipeline on a typical project, and what's your uniqueness/not-null coverage target?" A vendor who answers with a number (even an approximate one, like "roughly one test per model minimum") has real practice; "we write tests" without a number usually means a handful exist.
2. **Named freshness SLA breach response.** Ask what happens, concretely, when a table that should update daily hasn't updated in 30 hours — the answer should name an alerting tool and an escalation path, not "we'd notice."
3. **A specific schema drift catch.** Ask for one real example of a source schema change caught before it broke a downstream model — a vendor with production experience has a specific story, not a hypothetical.
4. **PII deletion walkthrough.** Ask them to trace, step by step, how a single individual's data gets located and deleted across raw, transformed, and exported layers — this single question exposes whether governance was designed in or bolted on.
5. **A dollar figure on a past cost optimization.** Ask for an actual number — "we cut a client's Snowflake bill by roughly 30% converting three full-refresh models to incremental" is a real answer; "we're always mindful of cost" is not.
...
### (Scenario: a board member questions a reconciliation mismatch) A board member just asked why last quarter's revenue dashboard number doesn't match this quarter's restated figure, and our data team can't explain it quickly — what does this reveal about our pipeline's design?
This is a symptom of missing lineage tracking and data quality observability, not a one-off bug — a well-instrumented pipeline lets someone trace a suspicious number back to its source and specific transformation step within minutes, not days. Treat this incident as the trigger to audit whether your current vendor or team has schema drift detection and anomaly alerting in place, since the next silent error is already accruing if this one went undetected for a full quarter.

### (Scenario: choosing between Airflow, Dagster, and Prefect for a new pipeline build) Our shortlisted vendors are split between recommending Airflow, Dagster, and Prefect for our new pipeline — how much should this specific tool choice weigh in our decision?
Less than whether the vendor has a reasoned position tied to your actual requirements — Airflow offers the deepest ecosystem, Dagster favors data-asset-centric abstractions increasingly popular for newer builds, and Prefect trades ecosystem maturity for a lighter operational footprint, and any of the three can be the right call. Weight the vendor's ability to explain why their recommendation fits your team's operational maturity and existing stack more heavily than the tool name itself.

### (Scenario: pipeline built without any dbt tests) We just learned our existing pipeline, built by a previous vendor, has zero automated dbt tests and relies entirely on someone eyeballing dashboards — how urgent is it to fix before bringing in a new vendor?
Treat this as an immediate risk rather than a backlog item, since a pipeline with no automated testing is actively producing unverified numbers right now, not just accumulating technical debt for later. Prioritize adding uniqueness, not-null, and referential integrity tests on your highest-visibility models first — the ones feeding executive dashboards — before any new feature work begins.

### (Scenario: pipeline moving EU customer data with unclear PII handling) We're about to onboard a data engineering vendor for a pipeline that will process EU customer PII, but they haven't mentioned field-level encryption or masking in their proposal — is that a dealbreaker?
It's at minimum a required clarifying question before signing, since a vendor who hasn't proactively addressed PII handling likely hasn't built GDPR-compliant governance into the architecture from the start. Ask them directly to walk through how a right-to-erasure request would be fulfilled across every downstream copy of a person's data — an unclear or improvised answer here is a genuine dealbreaker given the compliance liability it creates for you, not them.
...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    ...
    {"@type": "Question", "name": "(Scenario: a board member questions a reconciliation mismatch) A board member just asked why last quarter's revenue dashboard number doesn't match this quarter's restated figure, and our data team can't explain it quickly — what does this reveal about our pipeline's design?", "acceptedAnswer": {"@type": "Answer", "text": "This is a symptom of missing lineage tracking and data quality observability, not a one-off bug — a well-instrumented pipeline lets someone trace a suspicious number back to its source within minutes. Audit whether your current vendor or team has schema drift detection and anomaly alerting in place, since the next silent error is already accruing."}},
    {"@type": "Question", "name": "(Scenario: choosing between Airflow, Dagster, and Prefect for a new pipeline build) Our shortlisted vendors are split between recommending Airflow, Dagster, and Prefect for our new pipeline — how much should this specific tool choice weigh in our decision?", "acceptedAnswer": {"@type": "Answer", "text": "Less than whether the vendor has a reasoned position tied to your actual requirements — Airflow offers the deepest ecosystem, Dagster favors data-asset-centric abstractions, and Prefect trades maturity for a lighter footprint, and any can be right. Weight the vendor's ability to explain their recommendation more heavily than the tool name itself."}},
    {"@type": "Question", "name": "(Scenario: pipeline built without any dbt tests) We just learned our existing pipeline, built by a previous vendor, has zero automated dbt tests and relies entirely on someone eyeballing dashboards — how urgent is it to fix before bringing in a new vendor?", "acceptedAnswer": {"@type": "Answer", "text": "Treat this as an immediate risk, since a pipeline with no automated testing is actively producing unverified numbers right now. Prioritize adding uniqueness, not-null, and referential integrity tests on your highest-visibility models first, before any new feature work begins."}},
    {"@type": "Question", "name": "(Scenario: pipeline moving EU customer data with unclear PII handling) We're about to onboard a data engineering vendor for a pipeline that will process EU customer PII, but they haven't mentioned field-level encryption or masking in their proposal — is that a dealbreaker?", "acceptedAnswer": {"@type": "Answer", "text": "It's at minimum a required clarifying question before signing, since a vendor who hasn't proactively addressed PII handling likely hasn't built GDPR-compliant governance into the architecture. Ask them to walk through a right-to-erasure request across every downstream copy of a person's data — an unclear answer is a genuine dealbreaker."}}
  ]
}
</script>
