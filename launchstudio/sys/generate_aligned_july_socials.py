...
july_dir = '/Users/duyle/sickn33/launchstudio/2026/july-2026'
articles = sorted(glob.glob(f'{july_dir}/[0-9][0-9]-*.md'))
...
def process_and_write(fpath):
    ...
    if 'Dr. Visser' in ex_text: name = 'Dr. Visser'
        ...
        if m_name:
            ...
            m_name2 = re.search(r'([A-Z][a-z]+) (?:ran|built|launched|founded|owns|created|saw|had)', story_p)
            if m_name2:
    ...
    social_en = f"""🔥 {name} built a prototype using **{tool_name}** — {first_sent.lower() if first_sent else 'testing his new AI SaaS idea'}, but discovered critical architecture, security, and deployment bottlenecks before going live. 🧠
...
"""
    ...
    social_nl = f"""🔥 {name} bouwde een prototype met **{tool_name}** — {first_sent.lower() if first_sent else 'om een nieuw AI SaaS-idee te testen'}, maar ontdekte kritieke architectuur-, beveiligings- en deployment-knelpunten vóór de lancering. 🧠
...
"""
    ...
    en_path = os.path.join(july_dir, f'{base}-social.md')
    nl_path = os.path.join(july_dir, f'{base}-social_dutch.md')
...
print("Regenerating all 120 July social posts with exact Real Example alignment...")
for fpath in articles:
    process_and_write(fpath)
print("Done! All 60 EN and 60 NL July social posts are 100% aligned with main article Real Examples.")
