...
## A Quick Self-Test: Three Questions That Reveal Where You Actually Stand

Before committing to a full audit, three questions reliably reveal how far a given prototype actually is from production, regardless of how technically experienced its builder is. First: if you copied one of your app's own requests into a tool like Postman and changed nothing but the user ID in that request, would the response change to someone else's data — or get rejected? Second: search your own codebase, not just the current files but full git history, for the pattern "api_key" or "sk_" — does a real credential appear anywhere, ever, even in a line you later deleted? Third: disconnect your internet mid-request to any external service your app calls, and watch what the user actually sees — a clear message, or a frozen screen. Most founders can run all three checks in under fifteen minutes, and the honest answers tend to predict, with surprising accuracy, how much of the rest of this list still needs attention.
