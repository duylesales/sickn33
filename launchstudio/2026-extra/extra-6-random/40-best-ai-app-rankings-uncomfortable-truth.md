...
**Give it a follow-up instruction that conflicts with something it already built, and see what else changes.** Ask for a small, targeted adjustment to one feature, then check whether unrelated, previously-working code got silently touched in the process. A tool that changes exactly what you asked for, and nothing else, behaves very differently under iteration than one that quietly rewrites adjacent code every time you ask for a fix — and that difference compounds over months of real development in a way a single demo can't show you.
